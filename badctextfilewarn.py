...
def checkType(values):
    ...
    if v not in ('int', 'float', 'char'):
        raise BADCTextFileMetadataInvalid("Type not right must be int, float or char. not %s" % v)
...
class BADCTextFile:
    ...
    def check_valid(self):
        for label in BADCTextFile.MDinfo:
            ...
            if not applyg and self[label] != []:
                MetadataInvalid("Not allowed as global metadata parameter: %s, %s" %(label, self[label]))
            # if label can't apply to column but is defined raise error 
            if not applyc and self[label] == []:
                for colname in self.colnames():
                    if self[label,colname] != []: 
                        MetadataInvalid("Given metadata not allowed for a column: %s, %s, %s" %(label, colname, self[label,colname]))
            # values have wrong number of fields 
            for values in self[label]:
                if len(values) > maxo:
                    MetadataInvalid("Max number of metadata fields (%s) exceeded for %s: %s" % (maxo, label, values))
                if len(values) < mino:
                    MetadataInvalid("Min number of metadata fields (%s) not given for %s: %s" % (mino, label, values,))
            for colname in self.colnames():
                for values in self[label,colname]:
                    if len(values) > maxo:
                        MetadataInvalid("Max number of metadata fields (%s) exceeded for column:%s %s: %s" % (maxo, colname, label, values,))
                    if len(values) < mino:
                        MetadataInvalid("Min number of metadata fields (%s) not given for column:%s %s: %s" % (mino, colname, label, values,))
    ...
    def check_colRefs(self):
        ...
        if len(long_namesCnt) == len(metadataRefs):
            try:
                ...
                MetadataInvalid('Column name %s not used as reference to connect metadata entries %s'% (colName,','.join(metadataRefs)))
        else:
            MetadataInvalid('Not all column headings given. Header references are: %s\n Column headings given are: %s'% (','.join(metadataRefs),','.join(self.colnames())))            
        ...
        for label in BADCTextFile.MDinfo:
            ...
            if applyg and applyc:
                ...
                for colname in self.colnames():
                    ...
                    if mand == mandb:
                        MetadataInvalid("Mandatory basic global/column metadata not provided: %s" % label)
                    elif mand == mandc:
                        MetadataInvalid("Recommended complete global/column metadata not provided: %s" % label)
                    else:
                        MetadataInvalid("Suggested global/column metadata not provided: %s" % label)
                ...
                for colname in self.colnames():
                    ...
                    if mand == mandb:
                        MetadataInvalid("Mandatory global metadata not provided: %s" % label)
                    elif mand == mandc:
                        MetadataInvalid("Recommended complete global metadata not provided: %s" % label)
                    else:
                        MetadataInvalid("Suggested global metadata not provided: %s" % label)
            ...
            # each variable
                ...
                for colname in self.colnames():
                    if self[label,colname] == []: 
                        MetadataInvalid('Recommended complete column metadata "%s" for column %s missing'  % (label, colname))
    ...
    def NASA_Ames(self):
        ...
        # number of variables not co-ord variable
